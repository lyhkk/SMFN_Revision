...
class mse_weight(torch.nn.Module):
    def __init__(self):
        super(mse_weight, self).__init__()

    def forward(self, x, y):
        equ = torch.zeros((y.size(2), y.size(3))).cuda(0)
        N = y.size(2)
        for j in range(0, y.size(2)):   # row
                val = math.pi / N
                equ[j, :] = math.cos((j - (N/2) + 0.5) * val)
...
class TrainsetLoader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        idx_frame = random.randint(0, 31)
        lr_dir = self.trainset_dir_lr + '/' + self.video_list[idx_video]
        hr_dir = self.trainset_dir_hr + '/' + self.video_list[idx_video]
        # read HR & LR frames
        if idx_frame < 30:
            LR0 = Image.open(lr_dir + '/input4/' + str(idx_frame).rjust(3, '0') + '.png')
            LR1 = Image.open(lr_dir + '/input4/' + str(idx_frame + 1).rjust(3, '0') + '.png')
            LR2 = Image.open(lr_dir + '/input4/' + str(idx_frame + 2).rjust(3, '0') + '.png')
            HR0 = Image.open(hr_dir + '/truth/' + str(idx_frame).rjust(3, '0') + '.png')
            HR1 = Image.open(hr_dir + '/truth/' + str(idx_frame + 1).rjust(3, '0') + '.png')
            HR2 = Image.open(hr_dir + '/truth/' + str(idx_frame + 2).rjust(3, '0') + '.png')
        elif idx_frame < 32:
            LR0 = Image.open(lr_dir + '/input4/' + str(idx_frame - 2).rjust(3, '0') + '.png')
            LR1 = Image.open(lr_dir + '/input4/' + str(idx_frame - 1).rjust(3, '0') + '.png')
            LR2 = Image.open(lr_dir + '/input4/' + str(idx_frame).rjust(3, '0') + '.png')
            HR0 = Image.open(hr_dir + '/truth/' + str(idx_frame - 2).rjust(3, '0') + '.png')
            HR1 = Image.open(hr_dir + '/truth/' + str(idx_frame - 1).rjust(3, '0') + '.png')
            HR2 = Image.open(hr_dir + '/truth/' + str(idx_frame).rjust(3, '0') + '.png')
        ...
        LR1 = rgb2y(LR1)
        LR2 = rgb2y(LR2)
        #print(np.shape(HR0)) #720 1272
        #print(np.shape(LR0)) #180 318     
        ...
        #print(np.shape(HR0)) #128 128 -> 因为patch_size
        #print(np.shape(LR0)) # 32 32
        ...
        #print(np.shape(HR0)) #128 128 1 1
        #print(np.shape(LR0)) #32 32 1 1
...
class ValidationsetLoader(Dataset):
    def __init__(self, validate_dir_hr, validate_dir_lr):
        ...
        #self.n_iters = 100 * len(self.video_list)
        self.n_iters = 10 * len(self.video_list)
    ...
    def __getitem__(self, idx):
        ...
        if self.idx_frame < 30:
            LR0 = Image.open(lr_dir + '/input4/' + str(self.idx_frame).rjust(3, '0') + '.png')
            LR1 = Image.open(lr_dir + '/input4/' + str(self.idx_frame + 1).rjust(3, '0') + '.png')
            LR2 = Image.open(lr_dir + '/input4/' + str(self.idx_frame + 2).rjust(3, '0') + '.png')
            HR1 = Image.open(hr_dir + '/truth/' + str(self.idx_frame + 1).rjust(3, '0') + '.png')
        elif self.idx_frame < 32:
            LR0 = Image.open(lr_dir + '/input4/' + str(self.idx_frame - 2).rjust(3, '0') + '.png')
            LR1 = Image.open(lr_dir + '/input4/' + str(self.idx_frame - 1).rjust(3, '0') + '.png')
            LR2 = Image.open(lr_dir + '/input4/' + str(self.idx_frame).rjust(3, '0') + '.png')
            HR1 = Image.open(hr_dir + '/truth/' + str(self.idx_frame - 1).rjust(3, '0') + '.png')
        '''
        if idx % 100 == 0:
            self.idx_video = self.idx_video + 1
        ...
        elif self.idx_frame < 101:
            LR0 = Image.open(lr_dir + '/' + str(self.idx_frame - 2).rjust(3, '0') + '.png')
            LR1 = Image.open(lr_dir + '/' + str(self.idx_frame - 1).rjust(3, '0') + '.png')
            LR2 = Image.open(lr_dir + '/' + str(self.idx_frame).rjust(3, '0') + '.png')
            HR1 = Image.open(hr_dir + '/' + str(self.idx_frame - 1).rjust(3, '0') + '.png')
        W, H = LR1.size
...
parser.add_argument('--tr_dataset_hr', type=str, default='data1/VR-super-resolution/test/VR/GT') #train
parser.add_argument('--tr_dataset_lr', type=str, default='data1/VR-super-resolution/test/VR/LR') #train
parser.add_argument('--val_dataset_lr', type=str, default='data1/VR-super-resolution/test/VR/LR') #val
parser.add_argument('--val_dataset_hr', type=str, default='data1/VR-super-resolution/test/VR/GT') #val
args = parser.parse_args()


if __name__ == '__main__':
    tr_set = TrainsetLoader(args.tr_dataset_hr, args.tr_dataset_lr, 4, patch_size=32, n_iters=1000) # n_iters=2000
    val_set = ValidationsetLoader(args.val_dataset_hr, args.val_dataset_lr)
    test_set = TestsetLoader(args.val_dataset_hr, args.val_dataset_lr)
    testLoader = DataLoader(dataset=val_set, batch_size=1, shuffle=False)
